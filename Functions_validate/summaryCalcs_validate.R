...
summaryCalcs_validate = function(UseClass, Category, Indicator, CharacteristicNames, Units, valueType, standardType, standard){
...
# # #   
  ...
  numericalData = retrieveNumericalData_validate(Data, CharacteristicNames, Units, valueType) #data of interest formatted as numerical time series
  categoricalData = retrieveCategoricalData_validate(Data, CharacteristicNames, Units, valueType) #data of interest formatted as categorical time series 
  ...
  Severity = if (n > 0 & standardType == "severity") {
    findSeverity_validate(categoricalData$ResultMeasureValue)
  } else ""
  
  DateOfMax = if (Maximum > 0) {
    maxDate_validate(CharacteristicNames, Units, valueType, Maximum) 
  } else ""
...
if (n > 0 & standardType != "severity" & standardType != "manual"){
  trendParameters = calculateTrend_validate(numericalData)
  ...
  computeStandards = if (n > 0 & standardType == "severity"){
    ...
    function_call = paste(c(standardType, "_eval_validate", "(Severity)"), collapse="") #evaluate on the Severity ranking
    ...
    #read in the type of function required as indicated by the standardType
    function_call = paste(c(standardType, "_eval_validate", "(numericalData, standard)"), collapse="") #evaluate on the numerical data
    eval(parse(text=function_call)) #execute the appropriate function
