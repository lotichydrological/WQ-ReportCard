...
two_month_geometric_mean_eval_validate = function(Data, standard){
  ...
  if (nrow(Data) == 1) {
    ...
    rollingMean = rollapply(regularSeries, 60, FUN = function(x) geometricMean_validate(x), by = 60, fill = NA) #calculate a 60 day geometric mean over fixed two month intervals
